...
/*** Perform sanity checks on the input */
  ...
  FROM = malloc(DENSITY  * sizeof(NODE)    );	/* origin of each arc */
  TO   = malloc(DENSITY  * sizeof(NODE)    );	/* destination */
  U    = malloc(DENSITY  * sizeof(CAPACITY));	/* capacity */
  C    = malloc(DENSITY  * sizeof(COST)    );	/* cost */
  B    = malloc(NODES    * sizeof(CAPACITY));	/* supply (demand) at each node */

  pred = malloc(DENSITY * sizeof(NODE));
  head = malloc(DENSITY * sizeof(NODE));
  tail = malloc(DENSITY * sizeof(NODE));
 ...
 ***/
  ...
  for (i = NODES-SOURCES-SINKS; i > (4*(NODES-SOURCES-SINKS)+9)/10; i--) {
    node = choose_index(handle, (INDEX)randomr(1L, (long)index_size(handle)));
...
/* changing to handle overflows with large n; Mar 18 -- jc */
    ...
    for (i = 0; i < sinks_per_source; i++) {
      sinks[i] = choose_index(handle, (INDEX)randomr(1L, (long)index_size(handle)));
    }
    ...
    for (i = 0; i < sinks_per_source; i++) {
      sort_count++;
      partial_supply = randomr(1L, (long)supply_per_sink);
      j = randomr(0L, (long)sinks_per_source - 1);
      tail[sort_count] = k;
      head[sort_count] = sinks[i] + 1;
      B[sinks[i]] -= partial_supply;
      B[sinks[j]] -= (supply_per_sink - partial_supply);
      k = source;
      for (j = randomr(1L, (long)chain_length); j > 0; j--)
	k = pred[k];
    }
	...
	cap = SUPPLY;
	if (randomr(1L, 100L) <= CAPACITATED)
	  cap = MAX(B[source-1], MINCAP);
	cost = MAXCOST;
	if (randomr(1L, 100L) > HICOST)
	  cost = randomr(MINCOST, MAXCOST);
...
CAPACITY supply;
{
  ...
  for (i = 0; i < sources; i++) {
    B[i] += (partial_supply = randomr(1L, (long)supply_per_source));
    B[randomr(0L, (long)(sources - 1))] += supply_per_source - partial_supply;
  }
  B[randomr(0L, (long)(sources - 1))] += supply % sources;
}
...
long parms[];
{
  ...
  for (source = 1; source <= NODES/2; source++) {
    index = choose_index(skeleton, (INDEX)randomr(1L, (long)index_size(skeleton)));
    SAVE_ARC(source, index, randomr(MINCOST, MAXCOST), 1);
...
/* changing to handle overflows with large n; Mar 18 -- jc */
  ...
  for ( ; limit > 0; limit--) {
    index = choose_index(handle, (INDEX)randomr(1L, (long)pseudo_size(handle)));
    cap = SUPPLY;
    if (randomr(1L, 100L) <= CAPACITATED)
      cap = randomr(MINCAP, MAXCAP);

/* adding Aug 29 -- jc */
if ((1 <= index) && (index <= NODES)) {
    SAVE_ARC(desired_tail, index, randomr(MINCOST, MAXCOST), cap);
    }
