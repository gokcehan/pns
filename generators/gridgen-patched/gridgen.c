...
struct stat_para {    /* structure for statistical distributions */
              int64_t distribution;    /* the distribution */
              float parameter[5];  /* the parameters of the distribution */
                };

struct arcs {
              int64_t from,         /* the FROM node of that arc */
...
int64_t  exponential(),		/* Returns an exponentially distributed
...
int64_t    seed_original,     /* The original seed from input */
...
int64_t parameters[]={0,2,1};  /* number of parameters of various distributions */
...
int main(argc,argv)
  ...
  char **argv;
{
  int64_t batch_loop=1;  /* number of jobs in batch */
...
void assign_capacities()
{
  ...
  int64_t (*random)(),		/* Pointer to the appropriate random number
				   generating function */
      i;
...
void assign_imbalance()
{
  ...
  do
  {
    ...
    {
      ptr->supply = (int64_t)(randy()*avg + 0.5);
      total -= ptr->supply;
    }
...
int64_t exponential(lambda)  
  float *lambda;
{
  return ((int64_t)(- *lambda * log((double)randy()) + 0.5));
}
/*----------------------------------------------------------------------*/
...
/* this function is called at the beginning of each job, and returns
   SUCCESS or FAIL */
int64_t initialization(argc,argv,loop)  
  int64_t argc, *loop;
  char **argv;
{
  ...
  switch (arc_costs.distribution)
  {
    ...
    case UNIFORM :
      fprintf(outfile,"c arc costs: UNIFORM distr. min %d max %d\n",
	      (int64_t)*(arc_costs.parameter+0), (int64_t)*(arc_costs.parameter+1));
    break;

    case EXPONENTIAL :
      fprintf(outfile,"c arc costs: EXPONENTIAL distr. lambda %d\n",
	      (int64_t)*(arc_costs.parameter+0));
    break;
  }
  switch (capacities.distribution)
  {
    ...
    case UNIFORM :
      fprintf(outfile,"c arc caps :  UNIFORM distr. min %d max %d\n",
	      (int64_t)*(capacities.parameter+0), (int64_t)*(capacities.parameter+1) );
    break;

    case EXPONENTIAL :
      fprintf(outfile,"c arc caps :  EXPONENTIAL distr. %d lambda %d\n",
	      (int64_t)*(capacities.parameter+0) );
    break;
  }
...
void select_source_sinks()
{
  ...
  *ab = 0.9;
  ab[1] = n_node - 0.99;	/* upper limit is n_node-1 because
                                        the supernode cannot be selected */
  if ((temp_list=(int64_t *)malloc(n_node*sizeof(int64_t)))==NULL)
  {
    err(4);
    exit(1);
  }
...
{
  return ((int64_t)((a[1] - *a)*randy() + *a + 0.5));
}
/*----------------------------------------------------------------------*/
/*---------------------------- end of program --------------------------*/
